    ...
    <div class="vcb-table-panel relative flex">
      <table class="vcb-table" >
        <tr class="vcb-table-headers" >
          ...
          <th class="vcb-table-header">ផ្សេងៗ</th>
          <th class="vcb-table-header w-32">ទិន្នន័យ</th>
        ...
        <tr v-for="(record, index) in table.records.matched" :key='index' class="vcb-table-row" >
          ...
          <td class="vcb-table-cell" ><div v-html="applyTagMark(record.description)"></div></td>
          <td :class="'vcb-table-cell' + (parseInt(record.complete) == 1 ? ' text-green-600' : ' text-yellow-500' ) " >{{ parseInt(record.complete) == 1 ? "បញ្ចូលរួច" : "កំពុងបន្ត"  }}</td>
          <td class="vcb-table-actions-panel text-right" >
            <n-icon size="22" class="cursor-pointer text-blue-500" @click="showEditModal(record)" title="កែប្រែ" >
              <Edit20Regular />
            </n-icon>
            <n-icon size="22" class="cursor-pointer text-red-500" @click="destroy(record)" title="លុប" >
              <TrashOutline />
            </n-icon>
            <n-icon size="22" :class="'cursor-pointer ' + (record.active == 1 ? ' text-green-600 ' : ' text-gray-500 ') " @click="activateBook(record)" :title="record.active == 1 ? 'កំពុងដំណើរការ' : 'បានបិទដំណើរការ' " >
...
export default {
  ...
  setup(){
    ...
    function getDocumentTypes(){
      store.dispatch('bookType/compact').then(res=>{
        store.commit('bookType/setRecords',res.data.records)
      }).catch(err =>{
        notify.error({
