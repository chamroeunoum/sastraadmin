    ...
    <div class="vcb-table-panel relative flex">
      <table class="vcb-table" >
        ...
        <tr v-for="(record, index) in table.records.matched" :key='index' class="vcb-table-row" >
          ...
          <td class="vcb-table-cell" >{{ record.title }}</td>
...
export default {
  ...
  setup(){
    ...
    function getRegulators(query){
      ...
      store.dispatch('regulator/compact',{
        ...
        if(res.data.ok){
          ...
          regulators.value = res.data.records.map( 
            record => { 
              return {
    ...
    const regulatorName = computed( () => {
      return route.params.id > 0 ? store.getters['getRecord'].name : ''
    })
